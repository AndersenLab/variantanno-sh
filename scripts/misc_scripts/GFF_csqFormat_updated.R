...
gff_cb <- ape::read.gff("/vast/eande106/projects/Nicolas/fix_anno_ncbi/briggsae/c_briggsae_20250620_curated.gff")
...
write.table(update_cb,"/vast/eande106/data/c_briggsae/genomes/QX1410_nanopore/Feb2020/csq/QX1410.update.June2025.noWBGeneID.csq.gff3",quote = FALSE, row.names = FALSE, sep = '\t', col.names = FALSE)



# CGC1
gff_CGC <- ape::read.gff("/vast/eande106/data/c_elegans/genomes/CGC1/gene_models/CGC1.augustus.gff3")
...
update_CGC1 <- gff_CGC %>%
  ...
  # dplyr::mutate(attributes = ifelse(seqid == "MtDNA" & type == "CDS", gsub("Parent=", "Parent=transcript:", attributes), attributes)) %>%
  dplyr::mutate(attributes = ifelse(seqid == "MtDNA" & type == "gene", gsub("ID=", "ID=gene:", attributes), attributes)) %>%
  dplyr::mutate(seqid = gsub("chr","", seqid))


write.table(update_CGC1,"/vast/eande106/data/c_elegans/genomes/CGC1/gene_models/csq/CGC1.cleaned.Oct2025.csq.gff3", quote = FALSE, row.names = FALSE, sep = '\t', col.names = FALSE)
